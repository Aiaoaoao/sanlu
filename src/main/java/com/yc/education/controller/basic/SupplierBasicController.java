...
public class SupplierBasicController extends BaseController implements Initializable {
    ...
    @FXML private TextField supplier_pageSize;
    ...
    @FXML private TextField transaction_size;
    @FXML private TextField transaction_product_size;
    ...
    private int transactionProductSize = 10;
    ...
    public void findSupplierSearch(){
        String pageSizes =  supplier_pageSize.getText();
    ...
    public void supplierPages(MouseEvent event){
    ...
    public void loadMoreSupplier(int pageNum){
        ...
        List<SupplierBasic> supplierBasics = supplierBasicService.selectSupplierBasicNotSotp( stopCheckBox.isSelected() ? 1: 0,pageNum,pageSize);
        ...
        for (SupplierBasic supplierBasics1:supplierBasics) {
            ...
            if(supplierBasics1.getIsstop() == 1){
                supplierBasics1.setSupdes(supplierBasics1.getSupdes()+"(停用)");
            }
    ...
    public void findSupplier(int pageNum){
        ...
        List<SupplierBasic> supplierBasics = supplierBasicService.selectSupplierBasicNotSotp(0,pageNum,1);
        ...
        if(supplierBasics == null || supplierBasics.size() == 0){
            ...
            //权限管理
            matchingPermissions("供应商基本资料",insertvbox,deletevbox,updatevbox,printingvbox,clearvbox);
        }
    ...
    public void delete(){
        if(f_alert_confirmDialog(AppConst.ALERT_HEADER, AppConst.ALERT_DELETE)){
            ...
            //删除该供应商的联系人和负责人
            int rows =  supplierContactService.deleteSupplierContactBySupplierid(id);

            rows += supplierPurchaserService.deleteSupplierPurchase(id);

            rows += supplierBasicService.delete(id);
...
//        };
    ...
    public void loadMoreSupplierEmployee(int pageNum){
        ...
        List<EmployeeBasic> employeeBasics = employeeBasicService.selectEmployeeBasicByIdnum("".equals(order_textField.getText()) || order_textField.getText() == null ? "" :order_textField.getText(),pageNum,pageSize);
...
//        empid.setCellValueFactory(new PropertyValueFactory("id"));
   ...
   /*     ObservableList<SupplierPurchaser> list = tableView2.getItems();*/
        ...
        /*staffcode.setCellFactory((TableColumn<Object,Object> a ) -> new EditingCell<>());*/
            ...
            keycontent.setCellFactory((param) -> new TableViewSample.RadioButtonCell<SupplierPurchaserProperty, Participation>(EnumSet.allOf(Participation.class)));
...
//            keycontent.setCellValueFactory(new PropertyValueFactory("keycontent"));
             keycontent.setCellValueFactory(new PropertyValueFactory<SupplierPurchaserProperty,Participation>("participation"));
           ...
           if(supplierPurchasers.size()>0){
               for (SupplierPurchaser supplierPurchaser:supplierPurchasers) {
                   ...
                   if(supplierPurchaser.getKeycontent() == 1){
                        supplierPurchaserProperty = new SupplierPurchaserProperty(supplierPurchaser.getId(),supplierPurchaser.getStaffcode(),supplierPurchaser.getStaffname(),supplierPurchaser.getKeycontent()+"",supplierPurchaser.getStaffremarks(),Participation.设为默认,true);
                   }else {
...
//        tableView2.setOnMouseClicked(( t) -> {
//            if (t.getClickCount() == 2) {
//                moreSupplierEmployeeButtonClick();
//            }
//        });
        ...
        keycontent.setOnEditCommit(evt -> {
            ...
            for (SupplierPurchaserProperty supplierPurchaserProperty:supplierPurchaserProperties) {
                ...
                if(supplierPurchaserProperty.getId() == selected.getId() ){
                    ...
                    supplierPurchaserProperty.setParticipation(Participation.设为默认);
    ...
    public void contact(){
        ...
        mainContact.setCellFactory((param) -> new TableViewSample.RadioButtonCell<SupplierContactProperty, Participation>(EnumSet.allOf(Participation.class)));
...
//        new PropertyValueFactory<ShippingAddressProperty,Participation>("participation")
        mainContact.setCellValueFactory(new PropertyValueFactory<SupplierContactProperty,Participation>("participation"));
        ...
        mainContact.setOnEditCommit(evt -> {
            ...
            for (SupplierContactProperty supplierContactProperty:supplierContactProperties) {
                ...
                if(supplierContactProperty.getId() == selected.getId() ){
                    ...
                    supplierContactProperty.setParticipation(Participation.设为默认);
        ...
        for (SupplierPurchaserProperty supplierPurchaserProperty :supplierPurchaserProperties) {
            if(supplierPurchaserProperty.getStaffcode()!=null){
                ...
                if(supplierPurchaserProperty.getId()>0){
                    //修改采购负责人
                    SupplierPurchaser supplierPurchaser = new SupplierPurchaser(supplierPurchaserProperty.getId(),supplierPurchaserProperty.getStaffcode(),supplierPurchaserProperty.getStaffname(),keycontack,supplierPurchaserProperty.getStaffremarks());
                    supplierPurchaserService.updateNotNull(supplierPurchaser);
                }else{
                    //新增采购负责人
                    SupplierPurchaser supplierPurchaser = new SupplierPurchaser(supplierPurchaserProperty.getId(),1L,supplierPurchaserProperty.getStaffcode(),supplierPurchaserProperty.getStaffname(),keycontack,supplierPurchaserProperty.getStaffremarks(),0,supplierid);
    ...
    public void seachTransaction(){

        String transactionPageSize = transaction_size.getText();

        if(!"".equals(transactionPageSize) && transactionPageSize != null){
            TransactionSize = Integer.parseInt(transactionPageSize);
        }

        seachTransactionBillInfo();
...
//        List<ForceOrder> forceOrders = forceOrderService.findForceOrderBySupplier(supplierNum,"","");
        ...
        billemp.setCellValueFactory(new PropertyValueFactory("billemp"));
        totalprice.setCellValueFactory(new PropertyValueFactory("totalprice"));
            ...
            for (InquiryOrder inquiryOrder:inquiryOrders) {
                if(supplierBillInfos.size() == TransactionSize){
                    continue;
                }
                    SupplierBillInfo supplierBillInfo = new SupplierBillInfo(inquiryOrder.getCreatedate(),"询价单",inquiryOrder.getOrders(),"",inquiryOrder.getPurpeople(),inquiryProductService.findInquiryProductPriceSUM(inquiryOrder.getId()));
        ...
        for (PurchaseOrders purchaseOrder:purchaseOrders) {
            ...
            SupplierBillInfo supplierBillInfo = new SupplierBillInfo(purchaseOrder.getCreatedate(),"采购订单",purchaseOrder.getOrders(),purchaseOrder.getSeeorders(),purchaseOrder.getPurpeople(),purchaseProductService.findPurchaseProductPriceSUM(purchaseOrder.getId()));
        ...
        for (PurchaseStock purchaseStock:purchaseStocks) {
            ...
            SupplierBillInfo supplierBillInfo = new SupplierBillInfo(purchaseStock.getCreatedate(),"采购入库",purchaseStock.getStockorder(),"",purchaseStock.getInspectname(),purchaseStockProductService.findPurchaseStockProductPriceSUM(purchaseStock.getId()));
    ...
    public void searchTransationProduct(){
        String transactionProductSizeStr = transaction_product_size.getText();

        if(!"".equals(transactionProductSizeStr) && transactionProductSizeStr != null){
            transactionProductSize = Integer.parseInt(transactionProductSizeStr);
        }

        seachTransactionProductInfo();
    }
    ...
    public void seachTransactionProductInfo(){
        ...
        if(xjcheckedpro.isSelected() || !xjcheckedpro.isSelected() && !cgcheckedpro.isSelected() && !cgrkcheckedpro.isSelected() ){
            ...
            for (InquiryOrder inquiryOrder:inquiryOrders) {
                    if(supplierProductInfos.size() == transactionProductSize){
                        continue;
                    }
        ...
        if(cgrkcheckedpro.isSelected() || !xjcheckedpro.isSelected() && !cgcheckedpro.isSelected() && !cgrkcheckedpro.isSelected()){
            ...
            for (PurchaseStock purchaseStock :purchaseStocks) {
                ...
                for (PurchaseStockProduct purchaseStockProduct:purchaseStockProducts) {
                    ...
                    SupplierProductInfo supplierProductInfo = new SupplierProductInfo(purchaseStock.getCreatedate(),"采购入库",purchaseStock.getStockorder(),purchaseStockProduct.getPronum(),purchaseStockProduct.getProname(),purchaseStockProduct.getStocknum().toString(),purchaseStockProduct.getPrice()+"",purchaseStockProduct.getRemarks());
...
//        rightImage.setImage(image);
        ...
        if(StageManager.rightWin){
            ...
            stageStock.setTitle("供应商基本资料");
            Pane pane = new Pane();
            pane.getChildren().setAll(loader.load("/fxml/basic/loadMoreSupplier.fxml"));
            Scene scene = new Scene(pane);
    ...
    public void loadSupplierRight(boolean isStop){
        ...
        List<SupplierBasic> supplierBasics =  isStop ? supplierBasicService.selectSupplierBasicNotSotp(0) : supplierBasicService.selectSupplierBasicByIsStop(0);

        ObservableList<SupplierBasic> supplierBasicsList = FXCollections.observableArrayList();
        ...
        if(supplierBasics != null && supplierBasics.size() > 0){
            ...
            for (int i=0,len = supplierBasics.size();i < len;i++) {
                supplierBasics.get(i).setSort(i+1);
                if(supplierBasics.get(i).getIsstop() == 1){
                    supplierBasics.get(i).setSupdes(supplierBasics.get(i).getSupdes()+"(停用)");
                }
                supplierBasicsList.add(supplierBasics.get(i));
            }
        }

        rightSupplierTableView.setItems(supplierBasicsList);


        rightSupplierTableView.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<SupplierBasic>() {
            @Override
            public void changed(ObservableValue<? extends SupplierBasic> observableValue, SupplierBasic oldItem, SupplierBasic newItem) {
//               alert_informationDialog("=="+newItem.getId());

                SupplierBasic supplierBasic = supplierBasicService.selectByKey(newItem.getId());
